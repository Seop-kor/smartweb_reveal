$(function(){
  ...
  $(".description").each(function(index, item){
    ...
    $(window).resize(function(){
      ...
      // }
      if(winW < 900){
        $(".description").eq(index).text(a.substr(0,50));
      }else{
        $(".description").eq(index).text(a);
      }
    });
  });
  ...
  // function filterFn(casei) {
  //   $(".portfolio-box img").each(function(index, item) {
  //     $(this).show();
  //     if(casei === "all"){
  //       return;
  //     }
  //     if(item.dataset.tag != casei){
  //       item.style.display = "none";
  //     }
  //   });
  // };

  // $(".filter-menu .btns").on('click', function() {
  //   filterFn($(this).text().toLowerCase());
  // });
  let $grid = $('.portfolio-box').isotope({
  ...
  $('.filter-menu .btns').on( 'click', function() {
    $(".filter-menu .btns").removeClass("active");
